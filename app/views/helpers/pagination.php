...
**/
	function result($t="Résultats: ",$of=" sur ",$inbetween="-")
	{
		if (empty($this->_pageDetails)) { return false; }
		if ( !empty($this->_pageDetails['pageCount']) )
		{
...
**/
    ...
    function _generateUrl ($page=NULL,$pageDetails=NULL)
    {
		...
		foreach($getParams as $key => $value)
		{
			if ($pageDetails['paramStyle']=="get")
			{
				...
				{
					$getString = $this->_parseGetParams($key,$value, $getString);
				}
			}
			else
			{
    ...
    private function _parseGetParams($prefix, $next, $result) {
		$res = $prefix;
		if (is_array($next)){
			//Current
			$keys = array_keys($next);
			foreach ($keys as $key => $value) {
				$result = $this->_parseGetParams($res."%5B$value%5D",$next[$value], $result);
			}// foreach
		} else {
			$result[] = $res."=".$next;
			return $result;
